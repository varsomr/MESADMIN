(function () {
    ...
    angular
        .module('myApp')
        .controller('RetriveChkWebSpeccontroller', controller)
    ...
    function controller($scope, $timeout, uiGridConstants, OneStopSupportService) {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                { field: 'AttributeName', width: '20%', enableCellEdit: false },
     ...
     $scope.GridfromButton = function () {
        ...
        $scope.loadGrid = function () {
            ...
            if (document.getElementById('PWBS').value == null || document.getElementById('PWBS').value == "" || document.getElementById('PWBS').value == "PO" ) {
               wo_id = '1234567'; 
            }
            else {
                wo_id = document.getElementById('PWBS').value;
            }
            console.log("this is " + plant+ " and value "+wo_id);
            
            OneStopSupportService.getChkWebSpec(plant, wo_id).success(function (data) {
                if (data == null || data.ChkWebSpecList == null || data.ChkWebSpecList.length == 0) {
                    $scope.error = true;
                    $scope.errorDescription = "No data found for selected criteria.";
                } else {
                    $scope.gridOptions.paginationPageSizes.push(
                        data.ChkWebSpecList.length
                    );
                    var ChkWebSpecList = data.ChkWebSpecList;
                    $scope.gridOptions.data = ChkWebSpecList;
                    $scope.error = false;
                }
        ...
        $scope.gridOptions.onRegisterApi = function (gridApi) {
            ...
            $scope.exportDriver = function () {
                ...
                window.open('http://denm2008mesadm:93/api/Message/getChkWebSpec/' + document.getElementById('VATPlantOff').value + "/" + wo_id, '_blank', 'resizable=yes')
            }
